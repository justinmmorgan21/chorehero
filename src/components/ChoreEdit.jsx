...
export function ChoreEdit( { chore, currentParent, onClose } ) {
  ...
  useEffect(() => {
    ...
    currentParent.children.map( oneChild => {
      ...
      console.log("chore.children: ", chore.children);
      chore.children.forEach(choreChild => {
        // console.log("oneChild: ", oneChild);
        // console.log("choreChild: ", choreChild);
        if (choreChild.id === oneChild.id)
          initialChildCheckedStates[oneChild.id] = true;
        else
          initialChildCheckedStates[oneChild.id] = false;
      })
      // initialChildCheckedStates[oneChild.id] = chore.children.includes(oneChild);
    })
    ...
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <br />
          ...
          <div>
            <div>Assign chore to: </div>
            <div style={{display:"flex", flexDirection:"row"}}>
          {currentParent.children.map( (child) => (
            <div key={child.id} style={{marginRight:"12px"}}>
              <input type="checkbox" value={child.id}/> <label htmlFor={child}>{child.name}</label>
            </div>
          ))}
          </div>
            {chore.children.map((oneChild) => (
              <div key={oneChild.id}>
                {chore.child_chores.find(child_chore => child_chore.child_id === oneChild.id).active ? <div><input type="checkbox" name={oneChild.id} value={isChildChecked[oneChild.id]} onChange={()=>{
